...
Fundamentally - one of the primary objecties in the short-term is to define an implementation that provides the means for the [[ValuesCredentials]] to be supported via  [[HumanCentricAI]] related systems. The is consideredto be a critical requirements to support the [[WorkProject]] and in-turn address [[SocialAttackVectors]] and support [[SafetyProtocols]].
...
1.  The way in which the ‘Sem Web Ontologies’ are handled generally.
...
3.  Presently HTTP Uris are used for ontologies - DLT/DHT ontologies are useful; as such, the objective is to implement PermissiveCommons. 
